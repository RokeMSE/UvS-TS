...
class gwnet(nn.Module):
    ...
    def forward(self, A_wave, input):
        input = nn.functional.pad(input, (0, 0, 1, 0, 0, 0))
        B, N, T_in, Fe = input.shape  
        input = input.permute(0, 3, 1, 2)  # (B, N, T, F) → (B, F, N, T)
        in_len = input.size(3)
        ...
        x = self.end_conv_2(x)
        x = x.permute(0, 2, 1, 3)
        x = x.reshape(B, N, self.num_timesteps_output, Fe)
        return x