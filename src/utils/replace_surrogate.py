import torch
def replace_target(forget_target, surrogates, Df_indices, forget_indices, node_idxs, num_timesteps_input):
        ...
        for w in range(new_forget_target.shape[0]):
            ...
            for seg_idx, (f_start, f_end) in enumerate(forget_indices):
                ...
                if overlap_start < overlap_end:
                    ...
                    new_forget_target[w, node_idx, local_start:local_start+actual_len, :] = values_to_paste       
            
    return new_forget_target


def replace_dataset(dataset, surrogates, forget_indices, node_idxs, num_timestep_input):
    ...
    for i, node_idx in enumerate(node_idxs):
        ...
        for seg_idx, (f_start, f_end) in enumerate(forget_indices):
            ...
            new_dataset[node_idx, :, start:start+paste_len] = surrogate_seg[:, :paste_len]
