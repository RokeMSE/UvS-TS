...
class SATimeSeries:
    ...
    def unlearn_faulty_subset(self, dataset, forget_ex, faulty_node_idx, A_wave, means, stds, 
        ...
        print("TARGET: ", forget_target.shape)
        ...
        new_dataset = replace_dataset(dataset, [surrogate_data], forget_indices, [faulty_node_idx], num_timesteps_input, forget_output_mask)
        train_input, train_target = generate_dataset(new_dataset, num_timesteps_input, num_timesteps_output)
        retain_input, retain_target, Dr_indices, forget_input, forget_target, Df_indices = filter_forget(train_input, train_target, forget_indices, num_timesteps_input, num_timesteps_output)
        
        retain_loader = DataLoader(TensorDataset(retain_input, retain_target), batch_size=batch_size, shuffle=True)
        surrogate_data = DataLoader(TensorDataset(forget_input, forget_target), batch_size=batch_size, shuffle=True)
        
        history = self.training(surrogate_data, retain_loader, A_wave, 
    ...
    def unlearn_faulty_node(self, dataset, faulty_node_idx, A_wave, means, stds,
        ...
        impacts = VAR_IRF(dataset, faulty_node_idx)
...
def unlearn(model, A_wave, train_original_data, means, stds, num_timesteps_input, num_timesteps_output, test_loader, forget_set=None):
    ...
    if args.unlearn_node:
        history, retain_loader, forget_loader = sa_ts.unlearn_faulty_node(
            ...
            lambda_ewc=5.0, lambda_surrogate=0.05, lambda_retain=1.0, batch_size=32
        )
    else:
        history, retain_loader, forget_loader = sa_ts.unlearn_faulty_subset(
            ...
            lambda_ewc=5.0, lambda_surrogate=0.5, lambda_retain=1.0, batch_size=32
        )
    end.record()
    ...
    # Save eval results
    with open(path + f"/unlearn3_evaluation_results_{args.type}.txt", "w") as f:     
    ...
    torch.save({
        ...
        'history': history,
        'faulty_node_idx': args.node_idx
    }, args.model + f"/unlearn3_{args.type}_unlearned_model.pt")
